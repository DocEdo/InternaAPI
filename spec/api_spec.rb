# frozen_string_literal: true

require 'minitest/autorun'
require 'minitest/rg'
require 'rack/test'
require 'yaml'

require_relative '../app/controllers/app'
require_relative '../app/models/company'

def app
  Interna::Api
end

# get/ will simulate a call to the API and it will run the route to the API

DATA = YAML.safe_load File.read('app/db/seeds/company_seeds.yml')

describe 'Test Interna Web API' do
  include Rack::Test::Methods

  before do
    # Wipe database before each test
    Dir.glob("#{Interna::STORE_DIR}/*.txt").each { |comname| FileUtils.rm(comname) }
  end

  it 'Should find the root route' do
    get '/'
    _(last_response.status).must_equal 200
  end

  describe 'Handle companies' do
    it 'HAPPY: should be able to get list of all companies' do
      Interna::Company.new(DATA[0]).save
      Interna::Company.new(DATA[1]).save

      get 'api/v1/companies'
      result = JSON.parse last_response.body
      _(result['company_ids'].count).must_equal 2
    end

    it 'HAPPY: should be able to get details of a single company' do
      Interna::Company.new(DATA[1]).save
      id = Dir.glob("#{Interna::STORE_DIR}/*.txt").first.split(%r{[/.]})[3]

      get "/api/v1/companies/#{id}"
      result = JSON.parse last_response.body

      _(last_response.status).must_equal 200
      _(result['id']).must_equal id
    end

    it 'SAD: should return error if unknown company requested' do
      get '/api/v1/companies/foobar'

      _(last_response.status).must_equal 404
    end

    it 'HAPPY: should be able to create new companies' do
      req_header = { 'CONTENT_TYPE' => 'application/json' }
      post 'api/v1/companies', DATA[1].to_json, req_header

      _(last_response.status).must_equal 201
    end
  end
end
